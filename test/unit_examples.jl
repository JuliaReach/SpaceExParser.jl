@testset "Bouncing ball" begin
    ...
    assignment, guard = H.resetmaps[1][1], H.resetmaps[1][2]
...
@testset "Circle" begin
    ...
    # assignments in each mode 1, 2
    @test (H.resetmaps[1][1], H.resetmaps[2][1]) == ([:(x' = x), :(y' = y)], [:(x' = x), :(y' = y)])

    # guards in each mode 1, 2
    @test (H.resetmaps[1][2], H.resetmaps[2][2]) == ([:(y = 0)], [:(y >= 0)])
end
